...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrum-spacecoin directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(
                _('Electrum-Spacecoin datadir does not exist. Was it deleted while running?') + '\n' +
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-spacecoin.conf into user_config[]."""
    if not path:
        return {}
