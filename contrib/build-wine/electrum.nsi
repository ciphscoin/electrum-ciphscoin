...
;Variables
  ...
  !define PRODUCT_NAME "Ciphscoin Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/ciphscoin/electrum-ciphscoin"
  !define PRODUCT_PUBLISHER "Ciphscoin"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-ciphscoin-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-spacecoin\icons\electrum-ciphscoin.ico"
...
Section
  ...
  File "..\..\icons\electrum-ciphscoin.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-ciphscoin.ico" "0"

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe" 0


  ;Links spacecoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-ciphscoin.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-ciphscoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-ciphscoin.ico"
