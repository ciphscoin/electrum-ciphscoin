...
;Variables
  ...
  !define PRODUCT_NAME "Spacecoin Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/spaceworksco/electrum-spacecoin"
  !define PRODUCT_PUBLISHER "SpaceWorksCo"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-spacecoin-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-spacecoin\icons\electrum-spacecoin.ico"
...
Section
  ...
  File "..\..\icons\electrum-spacecoin.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-spacecoin.ico" "0"

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe" 0


  ;Links spacecoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum-spacecoin.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum-spacecoin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-spacecoin.ico"
